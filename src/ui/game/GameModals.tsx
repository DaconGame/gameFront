...
function OverlayButton({
  ...
  return (
    <button
      type="button"
      className={`h-[52px] w-[280px] border-2 bg-dungeon-stone text-xl text-bone-white transition hover:bg-dungeon-mid ${
...
export function PauseOverlay() {
  return (
    <div className="pointer-events-auto absolute inset-0 z-10 flex items-center justify-center bg-dungeon-deepest/75">
      <section className="flex w-[360px] flex-col items-center border-2 border-bone-white/90 bg-dungeon-deepest/95 px-8 py-9 shadow-[0_0_40px_rgba(0,0,0,0.75)]">
        <h2 className="text-3xl text-bone-white">일시정지</h2>
...
export function ResultOverlay({ result }: { result: HudResult }) {
  ...
  return (
    <div className="pointer-events-auto absolute inset-0 z-20 flex items-center justify-center bg-dungeon-deepest/85">
      <section className="flex w-[420px] flex-col items-center border-2 border-torch-core/90 bg-dungeon-deepest/95 px-8 py-9 shadow-[0_0_40px_rgba(0,0,0,0.75)]">
        <h2 className={result.victory ? "text-4xl text-torch-core" : "text-4xl text-blood-red"}>
          {title}
        </h2>
        <dl className="mt-8 grid w-full grid-cols-[1fr_auto] gap-x-5 gap-y-3 text-lg text-ash-grey">
          <dt>생존 시간</dt>
